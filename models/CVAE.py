...
class VAE(tf.keras.models.Model):
    def __init__(self, data_shape, latent_size=1024, batch_size=None, num_cond = None,
                 lr_init=1e-3, optimizer_type='adam', l2_reg=0, grad_threshold=None, only_initialize=None,
                 act_type='relu', do_bn=False, lr_scheduler_config=None, phase='inference', gm_mask=None, **kwargs):
        ...
        with tf.name_scope("Encoder"): 
            self.encoder = self.build_encoder(self.data_shape, self._latent_size, get_last_conv_shape=True, bn=self._do_bn, act=self._act_type)
        self.reparam_latent = self.latent_reparameterization()
        
        if self._num_cond is not None :
            self.decoder = self.build_decoder((latent_size+self._num_cond,), self.encoder.layers[-3].output_shape[1:], bn=self._do_bn, act=self._act_type)
        else :
            self.decoder = self.build_decoder((latent_size,), self.encoder.layers[-3].output_shape[1:], bn=self._do_bn, act=self._act_type)
    ...
    def call(self, input_x, input_cond=None, KL_beta=1, training=None):
        ...
        pix_loss = tf.reduce_mean(tf.keras.losses.MSE(y_pred=recon,
                                                      y_true=input_x))
        '''    
        #pix_loss = np.prod(input_x.shape[1:]) * tf.reduce_mean(tf.reduce_sum(tf.keras.losses.binary_crossentropy(input_x, recon), axis=(1, 2, 3)))
        pix_loss = np.prod(input_x.shape[1:]) * tf.reduce_mean(tf.reduce_sum(tf.keras.losses.MSE(y_pred=recon,y_true=input_x),-1))
        KL = self.kl_divergence(mu, sigma) * KL_beta
        loss = KL + pix_loss
        
        return mu, sigma, recon, loss, KL, pix_loss, corr
        
    def train(self, input_x, input_cond=None, KL_beta=1, numpy=False, lagging=False):
        self.encoder.trainable = False if lagging else True
        ...
        with tf.GradientTape() as tape:
            if input_cond is not None:
                mu, sigma, pred, loss, KL, pix_loss, corr = self(input_x, input_cond, KL_beta, training=True)
            else :
                mu, sigma, pred, loss, KL, pix_loss, corr = self(input_x, KL_beta, training=True)
        ...
        if numpy :
            return mu.numpy(), sigma.numpy(), pred.numpy(), loss.numpy(), KL.numpy(), pix_loss.numpy(), corr.numpy(), curr_lr.numpy()
        else : 
            return mu, sigma, pred, loss, KL, pix_loss, corr, curr_lr
    ...
    class build_conditional_input(tf.keras.layers.Layer):
        ...
        def call(self,input_x, input_cond):
            ...
            if self.__model == 'encoder':
                ...
                one_hot_cond = tf.one_hot(input_cond, depth=self.__num_cond,axis=-1)
                one_hot_cond = tf.reshape(one_hot_cond, [-1, 1, 1, 1, one_hot_cond.shape[-1]])
                for i,s in enumerate(input_x.shape[1:-1]):
                    one_hot_cond = tf.repeat(one_hot_cond, s, axis=i+1)
                
                input_x = tf.concat([input_x, one_hot_cond], axis=-1)
                return input_x
            
            if self.__model == 'decoder':
                input_cond = tf.squeeze(input_cond)
                one_hot_condition = tf.one_hot(input_cond, depth=self.__num_cond,axis=-1)
                if len(one_hot_condition.shape) != len(input_x.shape) :
                    one_hot_condition = tf.expand_dims(one_hot_condition,0)
                input_x = tf.concat((input_x, one_hot_condition),-1)
                return input_x
            
    # Build Encoder
    class build_encoder(tf.keras.Sequential):
        def __init__(self, input_shape, latent_size, filter_size_list=None, kernel_size_list=None, bn=False, act=None,
                     padding='same', use_bias=False, get_last_conv_shape=False):
            
            super(VAE.build_encoder, self).__init__()
            
            self.__get_last_conv_shape = get_last_conv_shape
            
            if not filter_size_list :
                filter_size_list = [32,64,128,128]
                #filter_size_list = [64,128,256,512]
            if not kernel_size_list :
                kernel_size_list = [3,3,3,3]
            
            self.add(tf.keras.layers.InputLayer(input_shape))
            for num_chn, k_size in zip(filter_size_list, kernel_size_list):
                conv1 = conv_block(k_size=k_size, s_size=1, num_chn=num_chn, padding=padding, 
                                  use_bias=use_bias, input_dims='3d', bn=bn, act=act)
                conv2 = conv_block(k_size=k_size, s_size=2, num_chn=num_chn, padding=padding, 
                                  use_bias=use_bias, input_dims='3d', bn=bn, act=act)
                self.add(conv1)
                self.add(conv2)
                
            self.add(tf.keras.layers.Flatten())
            self.add(fc_block(latent_size*2, use_bias=use_bias, bn=False, act=None))
    ...
    class build_decoder(tf.keras.Sequential):
        def __init__(self, input_shape, last_encode_conv_shape, filter_size_list=None, kernel_size_list=None, 
                     bn=False, act=None, padding='same', use_bias=False, get_last_conv_shape=False):
            
            #assert all([len(last_encode_conv_shape)==4, type(last_encode_conv_shape) is list])
            super(VAE.build_decoder, self).__init__()
            
            if not filter_size_list :
                filter_size_list = [128,128,64,32]
                #filter_size_list = [512,256,128,64]
            if not kernel_size_list :
                kernel_size_list = [3,3,3,3,]
                
            prev_node_num = 1
            for s in last_encode_conv_shape:
                prev_node_num*=s
            self.add(tf.keras.layers.InputLayer(input_shape))
            self.add(fc_block(prev_node_num, use_bias=use_bias, bn=bn, act=act))
            self.add(tf.keras.layers.Reshape(last_encode_conv_shape))
            
            for num_chn, k_size in zip(filter_size_list, kernel_size_list):
                deconv = deconv_block(k_size=3, s_size=2, num_chn=num_chn, padding=padding, 
                                      use_bias=use_bias, input_dims='3d', bn=bn, act=act)
                self.add(deconv)
                conv = conv_block(k_size=k_size, s_size=1, num_chn=num_chn, padding=padding, 
                                 use_bias=use_bias, input_dims='3d', bn=bn, act=act)
                self.add(conv)
                
                
            last_conv = conv_block(k_size=1, s_size=1, num_chn=1, padding=padding, use_bias=use_bias,
                                   input_dims='3d', bn=False, act=None)
            self.add(last_conv)
            self.add(tf.keras.layers.Cropping3D(cropping=((1, 0), (1, 0), (1, 0))))
    ...
    def kl_divergence(self, mean, sigma):
        ...
        logvar = tf.where(tf.math.is_nan(logvar), tf.ones_like(logvar)*(1/tf.keras.backend.epsilon()), logvar)
        logvar = tf.where(tf.math.is_inf(logvar), tf.ones_like(logvar)*tf.keras.backend.epsilon(), logvar)
        
        kl = 0.5 * ( mean** 2. + logvar - sigma -1)
        kl = tf.where(tf.math.is_nan(kl), tf.ones_like(kl)*(1/tf.keras.backend.epsilon()), kl)
        kl = tf.where(tf.math.is_inf(kl), tf.ones_like(kl)*tf.keras.backend.epsilon(), kl)
