...
class VAE(tf.keras.models.Model):
    def __init__(self, data_shape, latent_size=1024, batch_size=None, num_cond = None,
                 ...
                 act_type='relu', do_bn=False, lr_scheduler_config=None, phase='inference', **kwargs):
        ...
        self.encoder = self.build_encoder(self.data_shape, self._latent_size, get_last_conv_shape=True, act=self._act_type)
        self.reparam_latent = self.latent_reparameterization()
        
        if self._num_cond is not None :
            self.decoder = self.build_decoder((latent_size+self._num_cond,), (5,6,5,128), act=self._act_type)
        else :
            self.decoder = self.build_decoder((latent_size,), (5,6,5,128), act=self._act_type)
    ...
    def call(self, input_x, input_cond=None, KL_beta=1, training=None):
        ...
        mu, sigma, class_pred = self.encoder(input_dat, training)
        ...
        if KL <= 0 : KL = KL_beta
        if input_cond is not None :
            acc = self.accuracy(class_pred, input_cond)
            class_loss = self.Classification_Loss(input_cond,class_pred)
            loss = KL + pix_loss + class_loss
            return recon, loss, KL, pix_loss, corr, acc, class_loss
        else :
            loss = KL + pix_loss
            return recon, loss, KL, pix_loss, corr
        
    def train(self, input_x, input_cond=None, KL_beta=1, numpy=False):
        ...
        with tf.GradientTape() as tape:
            if input_cond is not None:
                pred, loss, KL, pix_loss, corr, acc, cn_loss = self(input_x, input_cond, KL_beta, training=True)
            else :
                pred, loss, KL, pix_loss, corr = self(input_x, KL_beta, training=True)
        ...
        if numpy :
            if input_cond is not None:
                return pred.numpy(), loss.numpy(), KL.numpy(), pix_loss.numpy(), corr.numpy(), curr_lr.numpy(), acc.numpy(), cn_loss.numpy()
            else : 
                return pred.numpy(), loss.numpy(), KL.numpy(), pix_loss.numpy(), corr.numpy(), curr_lr.numpy()
        else : 
            if input_cond is not None:
                return pred, loss, KL, pix_loss, corr, curr_lr, acc, cn_loss
            else : 
    ...
    class build_conditional_input(tf.keras.layers.Layer):
        ...
        def call(self,input_x, input_cond):
            ...
            if self.__model == 'encoder':
                ...
                cond_input = tf.concat([tf.ones_like(input_x, tf.float32) for _ in range(self.__num_cond)],-1)
    ...
    class build_encoder(tf.keras.Sequential):
        def __init__(self, input_shape, latent_size, filter_size_list=None, kernel_size_list=None, bn=False, act=None,
            ...
            self._num_cond = input_shape[-1]-1
            ...
            self.add(fc_block(latent_size*2+self._num_cond, use_bias=use_bias, bn=bn, act=None))
        ...
        def call(self, inputs, training=None):
            ...
            mu = output[:,:self._latent_size]
            sigma = output[:,self._latent_size:self._latent_size*2]
            class_pred = tf.nn.sigmoid(output[:,self._latent_size*2:self._latent_size*2+self._num_cond])
            return mu, sigma, class_pred
    ...
    def accuracy(self, x, y):
        if x.shape[-1] != 1 : x = tf.argmax(x,-1)
